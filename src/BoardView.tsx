...
const durOf = (s: Shot) => s.sourceDuration ?? 10; // SPEC-008: BEAT_SECONDS = 10
...
export default function BoardView({ ps }: { ps: PS }) {
  ...
  // SPEC-008 pipeline state (phase, characters, scenes, tracers, sound, finals)
  ...
  async function onAddBeat() {
    ...
    try {
      ...
      setDialog(created.id); // SPEC-008: add beat drops you straight into the text prompt
    } catch (e: any) {
  ...
  async function saveEdit(shotId: string) {
    ...
    p.setBoard(sb);
    mut((pl) => markBeatEdited(pl, shotId)); // SPEC-008 §6 — downstream artifacts go ⚠ stale
  ...
  // SPEC-008 Phase 1 exit: "done" locks the beat list and fixes total runtime
  function markDone() {
    const p = psRef.current;
          ...
          {shots.map((s, i) => {
            ...
            return (
              <div
                ...
                onClick={() => {
                  setSelected(s.id);
                  setDialog(s.id); // SPEC-008 §6 — pressing a beat opens its pipeline dialog
                }}
              >
...
function ReleasePlayer({
  ...
  // ponytail: no per-clip reseek — server ffmpeg mux is the frame-accurate path
  useEffect(() => {
