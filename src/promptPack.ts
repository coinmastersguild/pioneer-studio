...
import { buildFinalPrompt, extOf, geographyPrompt, motionSummary, refIntentOf, worldLayoutOf, type Pipeline } from "./pipeline";
...
export function shotBible(sb: Storyboard, pipe: Pipeline) {
  return {
    ...
    world: worldLayoutOf(pipe),
    locations: (pipe.locations || []).map((location) => ({
      id: location.id,
      name: location.name,
    ...
    beats: sb.shots.map((s, i) => {
      ...
      return {
        ...
        location: (pipe.locations || []).find((location) => location.id === ext.locationId)?.name || null,
        geography: ext.geography || null,
        geographyPrompt: geographyPrompt(pipe, ext) || null,
        ...
        finalPrompt: ext.finalPrompt || buildFinalPrompt(s.prompt, ext, pipe),
        narration: ext.audioCues || [],
...
export function promptPackMarkdown(sb: Storyboard, pipe: Pipeline): string {
  ...
  if (bible.world.name) {
    L.push("## World geography", "", `**${bible.world.name}** — ${bible.world.description}`, "");
    if (bible.world.rules.length) L.push("Continuity rules:", ...bible.world.rules.map((rule) => `- ${rule}`), "");
    if (bible.world.forbiddenElements.length) L.push(`Forbidden inventions: ${bible.world.forbiddenElements.join(", ")}`, "");
    for (const location of bible.locations) {
      L.push(`- **${location.name}** (${location.kind}) → ${location.adjacentTo.join(", ") || "no connections set"}`);
    }
    L.push("");
  }
  ...
  for (const b of bible.beats) {
    ...
    if (b.geography) L.push(`- Route: ${b.geography.entryFromId || "here"} → ${b.location} → ${b.geography.exitToId || "here"} · ${b.geography.movement} · ${b.geography.screenDirection}${b.geography.anchor ? ` · ${b.geography.anchor}` : ""}`);
    L.push(`- Reference intent: ${b.refIntent}`);
    if (b.motion) L.push(`- Motion: ${b.motion}`);
    if (b.cameraMove) L.push(`- Camera: ${b.cameraMove}`);
    for (const cue of b.narration) L.push(`- 🎙 ${cue.speaker} @ ${Number(cue.at).toFixed(1)}s — "${cue.text}"`);
