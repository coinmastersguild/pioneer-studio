// SPEC-008 pipeline: client-side state for phases 2–3 (characters, scenes,
// tracers, sound, finals) plus every generation helper. State persists in
// localStorage per storyboard id.
// ponytail: localStorage until the server doc-merge PATCH lands — swap
// load/save for the PATCH call then, shapes are already the doc shapes.
...
// SPEC-008 rev: locations are a shared library (like characters) — one driving
...
export type BeatExt = {
  ...
  staleFinal?: boolean; // SPEC-008 §6 — upstream edited after the final rendered
};
export type Pipeline = {
  ...
  release?: { url: string; duration: number; createdAt: string } | null; // last server master (SPEC-008 §2)
};

// SPEC-008 §6 staleness: upstream edits flag downstream artifacts ⚠ (kept, not
...
export function loadPipeline(id: string): Pipeline {
  ...
  return {
    phase: (Number(localStorage.getItem("ps_phase")) as 1 | 2 | 3) || 1, // migrate M1's key
    characters: [],
    locations: [],
    beats: {},
    musicPrompt: "",
...
// TTS: jobs model if one is listed, else the direct SPEC-003 /api/v1/tts route
// (verified live 2026-07-09: returns raw audio/wav bytes).
//
// Both take `text`, NOT `prompt` — voxcpm2-tts.tts 400s on a prompt param, so
// every voice line failed until this was fixed (2026-07-20). `voice` is that
// model's optional `voice_description`: a natural-language voice design
// ("young rogue, quick and sarcastic"), passed through on both paths.
 ...
 *
 *  Measured against alpha 2026-07-20: TTFB 0.64s vs 1.86s for the buffered
 *  route on the same line — and unlike the buffered route it is FLAT in input
 *  length (a 700-char answer used to cost 20s to first byte). That flatness is
 *  why the caller no longer clause-chunks: there is nothing left to hide.
 *
 ...
 *
 *  ttsDirect hands back an Artifact, which for inline audio means encoding the
 *  wav to a base64 data: URL and having the caller fetch it back — ~0.4MB
 *  through FileReader and a base64 round trip, measured at over a second on a
 *  single line. Playback doesn't need a URL, so it skips both. */
...
/** Same VoxCPM2 model as the jobs path, minus the queue: no submit/poll cycle,
 *  no R2 round-trip, no credit charge — it just returns the wav bytes.
 *
 *  Measured 2026-07-20 against alpha: 1.38s for a short clause, 2.73s for a
 *  sentence, vs 6.84s end-to-end through the jobs path. Conversation uses this;
 *  ttsLine keeps the jobs path for the storyboard, where a persisted R2 asset
 *  is the point and three seconds does not matter. See EPIC-010 §S1. */
...
// ponytail: browser mix; move to a server ffmpeg job if runtime gets long
