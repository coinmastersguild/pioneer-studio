// CreateView — the character-creation flow, lifted from the DegenQuest zoo
// "character trinity" (ASSET-GEN-SOP) into the studio.
//
//   text → concept cluster → portrait → voice → talking-head → VRM
//
// Every stage is a real job against the live alpha models (flux-schnell,
// flux2-dev, voxcpm2-tts, wan-s2v) except the VRM, which a browser can't make
// itself — Meshy needs a secret key + a server (CORS). That last step calls the
// pioneer-server /api/v1/vrm route (Meshy image-to-3d → rig → meshy2vrm). The
// portrait is the shared concept image the zoo keys everything to: it drives the
// talking head AND the VRM, so the 3D model and the video are the same character.
//
...
export default function CreateView({ ps }: { ps: PS }) {
  ...
  async function genVrm() {
    ...
    const id = active.id;
    if (!flow?.vrm) return ps.toast("VRM service not available — set MESHY_API on pioneer-server");
  ...
  return (
    <div className="create-root">
      ...
      <StageShell n={5} title="VRM (3D model)" model={flow?.vrm ? "meshy → rig → meshy2vrm" : "service off"} dotClass={dot("vrm")} locked={!ready("vrm")} lockedMsg="Needs a portrait (step 2)">
        ...
        {flow && !flow.vrm && (
          <p className="sc-warn">
            The Meshy→VRM service isn't configured on the server yet (needs <code>MESHY_API</code>). You can still upload a .vrm.
          </p>
        )}
        {active.vrm && (
