...
export default function HeadView({ ps, active }: { ps: PS; active: boolean }) {
  ...
  // An authored VRM can export 90+ shapes (jane-v8 does). Listing them all
  ...
  useEffect(() => {
    ...
    const stockReady = makeGltfLoader(renderer)
      ...
      .then((gltf) => {
        ...
        head.traverse((o) => {
          ...
          // ponytail: mask ignored, not interpreted. The reference repo's
    ...
    renderer.setAnimationLoop(() => {
      ...
      if (rig) {
        // One lipsync engine for both rigs (EPIC-010 S2): VrmLipSync reads the
   ...
   *
   *  No clause chunking any more: the streaming route's time-to-first-byte is
   *  flat in input length (0.64s measured), so splitting the opener bought
   *  nothing and cost prosody. Falls back to the buffered wav route once if the
   *  stream fails, so a bad deploy on one route doesn't mute the head. */
    ...
    try {
      ...
      stopVadRef.current = segmentSpeech(ctx, stream, {
        start: () => {
          ...
          setHearing(true);
          chunks = [];
          // ponytail: a fresh recorder per utterance clips the first ~50ms,
