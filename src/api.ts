...
// ── Companies & Projects ──────────────────────────────────────────────
// A project = a saved Studio session (a Storyboard doc), owned either by the
// wallet (personal) or a company (shared with all members). The server gates
// company projects with checkMembership, so listing them "just works" for
// every member the moment they log in.
export type Company = {
...
// No-burn named company for testing. Grants no credits; you become the owner.
...
// Wallet login: SIWE-style challenge → personal_sign → JWT. The returned token
// is a Bearer the same as an sk-pioneer key (server accepts either), so the app
// just stores it in the same credential slot. Message format is verbatim from
// pioneer-server auth.service.ts verifyAndIssueToken — must match exactly.
...
export async function fetchResultUrl(apiKey: string, jobId: string): Promise<{ url: string; contentType: string }> {
  ...
  if (ct.includes("json")) {
    ...
    // ponytail: no R2 on this account → result comes back inline base64.
...
// ── Storyboard (SPEC-007) — one document per address, rev-based CAS ──
...
// The server-side SPEC-007 storyboard routes 404 on alpha (checked 2026-07-09)
// — they were never deployed. The doc lives locally instead, same shapes and
// same function signatures, so call sites don't change; generation still goes
// through the live jobs API. Swap back to fetch calls when the server ships.
...
function sbSave(sb: Storyboard): Storyboard {
  ...
  if (activeProject) {
    // Mirror to the server project. Fire-and-forget; server owns rev CAS. On a
    // 409 we'd be stale — for the spike, last-write-wins (a toast/reload is the
    // follow-up when two members edit the same company project simultaneously).
    // ponytail: no rev round-trip here — send undefined rev to skip CAS.
    fetch(`${API_BASE}/api/v1/projects/${activeProject.id}`, {
      method: "PUT",
      headers: { ...authHeaders(activeProject.apiKey), "content-type": "application/json" },
...
/* ── Motion: ARDY text-to-animation (SPEC-009) ────────────────────────────────
 * NOT a jobs model — its own service, proxied by pioneer-server to the
 * CF-Access-gated inference tunnel (`/motion` on inference.pioneers.dev).
 * Verified live 2026-07-16: ARDY-Core-RP-20FPS-Horizon40 on `cskel27`,
 * 10s of motion in ~0.36s. The skeleton it drives is exactly `ardySkeleton.ts`,
 * so its output applies with no retargeting.
...
/* ── Character creation flow (SPEC-009 companion) ─────────────────────────────
 ...
 * plus a server-side Meshy→VRM route for the last step (a browser can't call
 * Meshy directly — secret key + CORS). Mirrors the DegenQuest zoo "character
 * trinity" (ASSET-GEN-SOP): shared concept image → 3D model + voice + talking head.
 */
...
/** Is the server-side Meshy→VRM route available? (503 when MESHY_API unset.) */
