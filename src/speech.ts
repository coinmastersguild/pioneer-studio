...
// writing clause 2.
//
// Time-to-first-sound is set entirely by the FIRST chunk: VoxCPM2 generation
// scales with output length (measured: an 84-character sentence took 2.9s to
// start returning and produced 6.9s of audio, whereas a ~45-character clause
// starts in well under half that). Everything after the first chunk is voiced
// under cover of playback and never felt, so only the opener is cut short.
...
// Ported from the game's tuned production VAD (DegenQuest
// `game/agents/sdk/src/agent/voice.ts:135-164`) rather than written fresh —
// EPIC-010 §S1 says port, don't reinvent. Only change: it reads an AnalyserNode
// in float [-1,1] instead of LiveKit Int16 frames, so the threshold is the same
// number rescaled (350/32768).
...
// The game has this and my first port dropped it, which is what produced a log
// full of "Mm-hmm", "Yeah" and "Oh": a breath or a keyboard tap is ~100ms of
// level, and STT asked to transcribe 100ms of noise does not return "" — it
// returns a plausible filler word. A segment must last this long to be an
// utterance at all.
 ...
 *
 *  ponytail: setInterval, not an AudioWorklet. A backgrounded tab throttles
